...
mod ffi {
    ...
    pub struct CandidateHash {
        // the concrete type for the H256 type is a 32-byte array
        pub data: [u8; 32],
    }

    // FFI  representation for use in C++ of the messages defined above
    #[derive(Debug, Clone, PartialEq)]
    ...
    unsafe extern "C++" {
        include!("polkadot-core-primitives/include/lib.h");

    }
}

#[cfg(feature = "std")]
impl From<ffi::InboundDownwardMessage> for InboundDownwardMessage {
    fn from(value: ffi::InboundDownwardMessage) -> Self {
        Self {
            sent_at: value.sent_at as _,
            msg: value.msg,
        }
    }
}
...
#[cfg(feature = "std")]
impl From<ffi::CandidateHash> for CandidateHash {
    fn from(value: ffi::CandidateHash) -> Self {
        let hash = sp_core::H256::from_slice(&value.data[..]);
        Self(hash)
    }
}
